...
# ═══════════════════════════════════════════════════════════════ #
#                           CHART 3                              #
#       PnL Breakdown by Asset Class (Grouped Bar)               #
# ═══════════════════════════════════════════════════════════════ #
...
bars1 = ax.bar(x - w, cat_data["Realized"], w, label="Realized P/L", color=TEAL, edgecolor=TEAL + "88", linewidth=0.5)
bars2 = ax.bar(x,     cat_data["Unrealized"], w, label="Unrealized P/L", color=AMBER, edgecolor=AMBER + "88", linewidth=0.5)
bars3 = ax.bar(x + w, cat_data["Total"], w, label="Total P/L", color=CORAL, edgecolor=CORAL + "88", linewidth=0.5)

ax.set_xticks(x)
ax.set_xticklabels(cat_data.index, fontsize=10, rotation=15, ha="right")
ax.axhline(0, color=MUTED, linewidth=0.8)
ax.set_title("P&L Breakdown by Asset Class", fontsize=16, fontweight="bold", pad=15)
ax.set_ylabel("Profit / Loss (USD)", fontsize=12, fontweight="bold")
...
for bar, val in zip(bars3, cat_data["Total"]):
    ...
    ax.text(bar.get_x() + bar.get_width() / 2, y_pos, fmt_usd(val),
            ha="center", va=va, fontsize=8, fontweight="bold", color=GREEN if val >= 0 else RED)
...
fig, ax = plt.subplots(figsize=(12, max(5, len(df_top_bottom) * 0.55)))
...
for bar, val in zip(bars, df_top_bottom["Total P/L"]):
    ...
    ax.text(x_pos + offset, bar.get_y() + bar.get_height() / 2,
            fmt_usd_full(val), va="center", ha=ha, fontsize=9, color=TEXT_CLR, fontweight="bold")
...
for i, (label, value, color) in enumerate(kpis):
    ...
    ax_kpi.text(x + 0.125, y + 0.12, label, fontsize=10, ha="center", va="center",
                color=MUTED, fontweight="bold", transform=ax_kpi.transAxes)
    ax_kpi.text(x + 0.125, y - 0.02, value, fontsize=16, ha="center", va="center",
                color=color, fontweight="bold", transform=ax_kpi.transAxes)

ax_kpi.set_xlim(0, 1)
ax_kpi.set_ylim(0, 1)

fig.suptitle("Trade Statistics & Win Rate Analysis", fontsize=16, fontweight="bold", y=1.02)
fig.tight_layout()
...
for idx, grp in enumerate(sorted_groups):
    ...
    if len(series) > 0:
        ax.annotate(
            ...
            bbox=dict(boxstyle="round,pad=0.2", fc=BG, ec=color, alpha=0.85, linewidth=0.7),
...
ax.set_title("Cumulative P&L by Commodity Group Over Time", fontsize=17, fontweight="bold", pad=18)
ax.set_ylabel("Cumulative P&L (USD)", fontsize=12, fontweight="bold")
ax.yaxis.set_major_formatter(mticker.FuncFormatter(fmt_usd))
ax.grid(True, alpha=0.35)
ax.tick_params(axis="x", rotation=30)

# Legend sorted by performance, outside the plot area
ax.legend(loc="upper center", bbox_to_anchor=(0.5, -0.12), ncol=min(5, len(sorted_groups)),
          framealpha=0.95, fontsize=9, columnspacing=1.5, handlelength=2.5)
